...
A fresh `supabase db reset` / SQL editor apply of `001`→`009` recreates these policies.
...
| [`009_e2ee_push.sql`](../supabase/migrations/009_e2ee_push.sql) | `pre_key_bundles` (public read, owner write), `message_envelopes` (sender/recipient), `push_tokens` (owner), `post_reactions` (can_view_post). No `notifications` table. Realtime on envelopes. |
...
Automated coverage: [`apps/web/tests/rls/security.test.ts`](../apps/web/tests/rls/security.test.ts) and [`apps/web/tests/rls/social.test.ts`](../apps/web/tests/rls/social.test.ts) (CI).
